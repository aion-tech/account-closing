...
class MarcoImporter(models.TransientModel):
    ...
    def partners_bank_change(self):
        return self.on_change_check(
            condition=self.partners_bank and not (self.partners and self.banks),
            title="DIPENDENZA NELL'IMPORT",
            message="Le PARTNERS_BANKS dipendono dai PARTNERS e dalle BANKS, assicurati di averli già importati.",
        )
